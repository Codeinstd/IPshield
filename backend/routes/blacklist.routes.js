...
router.get("/", requireAuth, requireRole('readonly'), [
  ...
  const result = await listBlacklist({ severity, status, q, limit: parseInt(limit), offset: parseInt(offset) });
...
router.post("/", requireAuth, requireRole('analyst'), [
  ...
  const entry = await addToBlacklist({ ip, severity, category, reason, added_by, expires_at, tags });
...
router.put("/:id",requireAuth, requireRole('analyst'), [
  ...
  const entry = await updateBlacklist(id, req.body);
...
router.delete("/bulk", requireAuth, requireRole('analyst'), [
  ...
  const count = await bulkDelete(req.body.ids.map(Number));
...
router.delete("/:id", requireAuth, requireRole('analyst'), [
  ...
  const ok = await deleteFromBlacklist(parseInt(req.params.id));
