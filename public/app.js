(() => {
  ...
  casesBtn.style.cssText = "padding:6px 12px;font-size:11px;font-family:'Inter', sans-serif;";
...
`;
  ...
  async function loadBillingInfo() {
    try {
      ...
      const isPaid = data.plan === "team" || data.plan === "admin";
      const canceling = !!data.cancel_at_period_end;
 
      btn.textContent = planName.toUpperCase() + (canceling ? " ⚠" : "");
 
      const color = canceling ? "var(--medium)" : isPaid ? "var(--accent)" : "var(--low)";
      btn.style.color       = color;
      btn.style.borderColor  = color;
      btn.style.background = canceling ? "rgba(255,204,0,0.1)" : isPaid ? "rgba(0,217,255,0.1)" : "rgba(0,232,124,0.1)";
  ...
  // plan / render modal body 
    ...
    try {
      ...
      const isPaid = plan === "team" || plan === "admin";
      const canceling = !!data.cancel_at_period_end;
...
if (plan === "admin") {
  actionsHtml = `
    <div style="font-size:11px;color:var(--text3);line-height:1.6;">
      Local administrator account. Unlimited access enabled for development.
    </div>`;
} else if (!isPaid) {
  actionsHtml = `
      ...
      body.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:18px;">
          <div style="flex:1;">
            ...
            <div style="font-family:'Syne',sans-serif;font-size:22px;font-weight:800;color:${isPaid ? "var(--accent)" : "var(--low)"};">
